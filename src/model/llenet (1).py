...
class Mainnet(nn.Module):
    def __init__(self, args):
        ...
        self.lnet = Lnet(self.num_L+1)
        self.rnet = Rnet(self.num_R+3)
        self.l_stage = self.make_lnet(self.S, self.num_L+1)
...
#         self.frnet = Rnet(self.num_R+3) #finetune
        ...
        self.cnt = 0
        self.conv_l = nn.Conv2d(in_channels=1,out_channels=32, kernel_size=3, stride=1, padding=1,
...
#         )
#
#         self.conv2 = nn.Sequential(nn.Conv2d(in_channels=self.channels+1, out_channels=self.channels, kernel_size=1))
#
#     def forward(self,input):
#         conv1 = self.conv1(input)
#         conv2 = torch.cat((conv1, input[:,:1,:,:]), dim=1)
