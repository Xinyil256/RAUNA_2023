...
class Mainnet(nn.Module):
    ...
    def forward(self, input_r, input_l, a):
        b, _, h, w = input_r.shape
        al = a.unsqueeze(dim=1).unsqueeze(dim=2).unsqueeze(dim=3).expand(-1, 1, h, w).to(self.device)
        # print(al.shape)
...
class Lenet(nn.Module):
    def __init__(self):
        ...
        self.res1 = nn.Sequential(
            nn.Conv2d(in_channels=2, out_channels=32, kernel_size=3, padding=1, padding_mode='reflect', bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=32, out_channels=2, kernel_size=3, padding=1, padding_mode='reflect', bias=True),
        )
    ...
    def forward(self, input, scale):
        ...
        # print(scale)
        # print('.......................')
        input = torch.cat((input, scale), dim=1)
        res1 = self.relu(input + self.res1(input))  # dim=1+1
        out2 = self.relu(res1 + self.res2(res1))
...
class Renet(nn.Module):
    ...
    def forward(self, input, scale):
        input1 = torch.cat((input, scale), dim=1)
        out1 = self.relu(input1 + 1. * self.res1(input1))# dim=3+1
        out2 = self.relu(out1 + 1. * self.res2(out1))
        out3 = self.relu(out2 + 1. * self.res3(out2))
        out4 = self.relu(out3 + self.res4(out3))
        return out4[:,:3,:,:]
